...
namespace Simulator
{
	public partial class Form1 : Form
	{
        ...
        DataAcquisitionStation localBaseStation;
		...
		delegate void SetTextDelegateRumbo(double COG, int indiceLista);
		//0 = desconectado, 1 = conectado
		int SenderState;
		int edoSimulacion;

		int sendingInterval = 5;
		int integratorServerPort = 5001;
		//String integratorServerIpAddress = "192.168.1.104";
		String integratorServerIpAddress = "192.168.1.125";
		...
		{
			bool wantToSend = true;

			//TimeController temporizador = new TimeController(sendingInterval, true);

            WrappedNMEAZMQSender sender;
            sender = new WrappedNMEAZMQSender(integratorServerIpAddress, integratorServerPort);
			...
			while (true) 
			{
				...
				{
					//TODO Aqui dar feedback de enviando

                    Thread.Sleep(sendingInterval * 1000);

					for (int i = 0; i < listaObjetivos.Count; i++)					
					{
                        if (!sender.SendTarget(listaObjetivos[i]))
                            break;
                        //if (sender.aliveConecction())
                        //{
			...
			//salio del envio pero no por orden explicita
			//(se requiere reconectar pero las comunicaciones fallaron)
			if (wantToSend)
			{
				//TODO Aqui dar feedback de reconectando
		...
		private void ajustaRumbo(double COG, int indiceLista)
		{
			...
			{
                listaObjetivos[indiceLista].course = (float)COG;
		...
		{ 
			...
			if (SenderState == 0) //sender esta desconectado
			{
				...
				//iniciar envio
				//se requiere un hilo y un temporizador
                SendingThread = new Thread(envio);
                SendingThread.Start();
        ...
        private void btConvertirAGrados_Click(object sender, EventArgs e)
        {
            int g = Convert.ToInt32(tbGrados.Text);
            int m = Convert.ToInt32(tbMinutos.Text);
            int s = Convert.ToInt32(tbSegundos.Text);
            double GradosDecimales = geodesicUtils.coordinatesFromDMStoDecimalDegrees(g, m, s);
            tbGradosDecimales.Text = GradosDecimales.ToString();
        }
	}
}
